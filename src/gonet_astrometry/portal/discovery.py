"""Filesystem discovery helpers for GONet portal inputs.

Discovery is intentionally lightweight: it identifies candidate original GONet
``.jpg`` files from explicit file paths and directories without parsing their
pixel payloads. Native image loading is deferred until a file is selected for
inspection.
"""
...
def parse_source_paths(value: str | None) -> tuple[Path, ...]:
    """Parse one-path-per-line portal input into normalized path objects.

    Parameters
    ----------
    value
        Text entered in the portal source-path field. Newlines and semicolons
        separate paths; surrounding whitespace is ignored.

    Returns
    -------
    tuple[pathlib.Path, ...]
        Paths in user-provided order with empty entries removed.
    """
    if value is None:
        return ()
